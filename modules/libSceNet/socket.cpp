...
EXPORT SYSV_ABI int sceNetSend(SceNetId s, const void* msg, size_t len, int flags) {
  int _ret;
  if ((_ret = send(s, (const char*)msg, (int)len, flags)) == SOCKET_ERROR)
    return sce_WSAGetLastError();
  return _ret;
}
...
EXPORT SYSV_ABI int sceNetSendto(SceNetId s, const void* msg, size_t len, int flags, const SceNetSockaddr* to, SceNetSocklen_t tolen) {
  int _ret;
  if ((_ret = sendto(s, (const char*)msg, (int)len, flags, (const sockaddr*)to, (int)tolen)) == SOCKET_ERROR)
    return sce_WSAGetLastError();
  return _ret;
}
