...
EXPORT SYSV_ABI int32_t sceHmdReprojectionInitialize(SceHmdReprojectionResourceInfo* resource, SceHmdReprojectionReprojectionType type, void* option) {
  LOG_USE_MODULE(libSceHmd);
  LOG_ERR(L"todo %S", __FUNCTION__);
  return Ok;
}
...
EXPORT SYSV_ABI int32_t sceHmdGetDeviceInformationByHandle(int32_t handle, SceHmdDeviceInformation* info) {
  LOG_USE_MODULE(libSceHmd);
  LOG_ERR(L"todo %S", __FUNCTION__);
  return Ok;
}
...
EXPORT SYSV_ABI int sceHmdReprojectionSetDisplayBuffers(int32_t videoOutHandle, int32_t index0, int32_t index1, void* option) {
  LOG_USE_MODULE(libSceHmd);
  LOG_ERR(L"todo %S", __FUNCTION__);
  return Ok;
}

EXPORT SYSV_ABI int sceHmdReprojectionSetOutputMinColor(float r, float g, float b) {
  LOG_USE_MODULE(libSceHmd);
  LOG_ERR(L"todo %S", __FUNCTION__);
  return Ok;
}

EXPORT SYSV_ABI int32_t sceHmdOpen(int32_t userId, int32_t _type, int32_t index, const SceHmdOpenParam* pParam) {
  LOG_USE_MODULE(libSceHmd);
  LOG_ERR(L"todo %S", __FUNCTION__);
  return Ok;
}

EXPORT SYSV_ABI int32_t sceHmdGetFieldOfView(int32_t handle, SceHmdFieldOfView* fieldOfView) {
  LOG_USE_MODULE(libSceHmd);
...
EXPORT SYSV_ABI int32_t sceHmdGet2DEyeOffset(int32_t handle, SceHmdEyeOffset* leftEyeOffset, SceHmdEyeOffset* rightEyeOffset) {
  LOG_USE_MODULE(libSceHmd);
  LOG_ERR(L"todo %S", __FUNCTION__);
  return Ok;
}
